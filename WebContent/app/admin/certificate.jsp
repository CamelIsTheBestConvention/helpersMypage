          ...
          <div class="request-wrap">
            ...
            <div class="request-box">
              ...
              <table class="type03">
                ...
                <tbody>
                  ...
                  <c:choose>
                  	<c:when test="${not empty certificateLists and fn:length(certificateLists) > 0}">
                  		<c:forEach var="certificateList" items="${certificateLists}">
                  			<tr>
                  				<td><c:out value="${certificateList.getBoardId()}"/></td>
                  				<td><c:out value="${certificateList.getIndMemberName()}"/></td>
                  				<td><c:out value="${certificateList.getIndMemberAccount()}"/></td>
                  				<td><c:out value="${certificateList.getBoardTitle()}"/></td>
                  				<td><c:out value="${certificateList.getIndMemberMobile()}"/></td>
                  				<td><c:out value="${certificateList.getIndMemberEmail()}"/></td>
                  				<td><c:out value="${certificateList.getBoardCompletedDate()}"/></td>
                  				<td><button class="secession" onclick="location.href='/admin/deleteCertificate.admin?boardId=${certificateList.getBoardId()}'">삭제</button></td>
                  			</tr>
                  		</c:forEach>
                  	</c:when>
                  	<c:otherwise>
            ...
            <table class="paging-table" style="font-size:1.3rem">
        	    <tr align="center" valign="middle">
					<td class="">
						<c:if test="${prev}">
							<a href="${pageContext.request.contextPath}/admin/certificate.admin?page=${startPage - 1}">&lt;</a>
						</c:if>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${not (i eq page)}">
									<a href="${pageContext.request.contextPath}/admin/certificate.admin?page=${i}">
										<c:out value="${i}"/>&nbsp;&nbsp;
									</a>
								</c:when>
								<c:otherwise>
						...
						<c:if test="${next}">
							<a href="${pageContext.request.contextPath}/admin/certificate.admin?page=${endPage + 1}">&gt;</a>
						</c:if>
					</td>
				</tr>
			</table>
